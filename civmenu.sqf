...
if (Antwort == 1) then
	...
	if ((_geld + bankmoney) < _strafe) exitwith 
	...
	if (bankmoney > _strafe) exitwith {bankmoney = bankmoney - _strafe;};
...
if (_civkopfgeld != 0) then 
	...
	(format['if(rolestring == "%1")then{bankmoney = bankmoney + %2; player groupChat "This civ had a bounty of $%2! You got that bounty!"}', _copobj, _civkopfgeld]) call broadcast;
...
if(stolencash > 0)then
	...
	if(bankmoney < (stolencash - _INVrmv))then{_BNKrmv = bankmoney};	

	["geld", -(_INVrmv)] call INV_addinventoryitem;
	bankmoney = bankmoney - _BNKrmv;
