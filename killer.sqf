...

{
	...
	if (iscop or ("bounty" call INV_HasLicense and _civkopfgeld > 20000)) then 				//Ist der Mörder ein Cop oder ein Kopfgeldjäger
	{
		...
		(format['if (player == %3)then{bankmoney = bankmoney + %2; player groupChat "You got 1/4 of the civs bounty totaling $%2"}', player, _civkopfgeld, _self]) call broadcast;
		};
		...
		(format['if (player == %3)then{bankmoney = bankmoney + %2; player groupChat "You got 1/2 of the civs bounty totaling $%2"}', player, _civkopfgeld, _self]) call broadcast;
		};
