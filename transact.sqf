...
if (INV_PLAYERLIST select _transfer == player) exitWith 
	...
	if (_geld < _transfermoney) then 
		...
		bankmoney = bankmoney + _transfermoney;
...
if (bankmoney < _steuern) then 
	...
	bankmoney = bankmoney - _steuern;
	INV_SteuernGezahlt = (INV_SteuernGezahlt + (_steuern - _transfermoney));
	(format ["if (INV_ROLESTRING == ""%1"") then {bankmoney = bankmoney + %2; player groupChat format[localize ""STRS_bank_account_getmoney"", %2, %3];};", _targetString, (_transfermoney call ISSE_str_IntToStr), player]) call broadcast;
...
if (bankmoney >= _withdraw) then 

	{

	bankmoney = bankmoney - _withdraw;
